...
#'
#' This function is used to fit the generalized spatially varying coeffcient models.
#'
#' @importFrom MGLM kr
...
function(y, X, S, V, Tr, d = 2, r = 1, lambda = 10^seq(-6, 6, by = 0.5),
  ...
  # The triangulation is the same as the population triangulation that she defines
  # at the beginning of the simulation code. The reason that the basis is created
  # again is because the matrix S is sampled from the population so we will get 
  # different basis matrix. The beta that is calculated based on this data will be
  # different than mhat_all (in simulation code) that is based on the entire population.T
  # The beta calculated in gsvcm_est is for the simulated spatial location data matrix S.

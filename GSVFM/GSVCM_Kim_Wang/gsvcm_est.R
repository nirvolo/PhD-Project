...
gsvcm_est=function(y, X, BQ2, P, lambda, family, off = 0, theta = 0){
  ...
  for(il in 1:nl){
    ...
    mevg = mu.eta(etastart)   # Vector of 1s
    ...
    # I think that temp3 is the initial value of the objective function in her paper.
    temp3 = t(Z) %*% temp1 + lambda[il] * D
    # Setting tol = 1e-22 means when trying to solve for the inverse, it won't give an 
    # erorr if there are really small values.
    alpha_old = solve(temp3, t(Z) %*% temp2, tol = 1e-22)
    step = 0
    delta1 = 1
    delta2 = 1
    # I think that the while loop is the beginning of the PIRLS algorithm (algorithm 1) in the paper
    
    while(delta1 > 1e-5 & sum(is.infinite(delta2)) == 0 & step <= 10){
      step = step + 1
      ###### These are the updates for the current iteration ###########
      eta = Z %*% alpha_old + off
      mu = linkinv(eta)
      ...
      # Setting tol = 1e-22 means when trying to solve for the inverse, it won't give an 
      # erorr if there are really small values.
      alpha_new = solve(temp3, t(Z) %*% temp2, tol = 1e-22)
      eta_new = Z %*% alpha_new
      ...
      #### Debugging ###
      #print(c(delta1,delta2, step))
    }
    alpha_all[,il] = as.matrix(alpha_old)
    # Setting tol = 1e-22 means when trying to solve for the inverse, it won't give an 
    # erorr if there are really small values.
    DD = solve(temp3, tol = 1e-22)
  ...
  function(y, X, BQ2, P, lambda, family, off = 0, theta = 0, Cp = TRUE)
  {
    ...
    while(delta1 > 1e-5 & sum(is.infinite(delta2)) == 0 & q <= 20){
      ...
      for(il in 1:nl){
        ...
        # Setting tol = 1e-22 means when trying to solve for the inverse, it won't give an 
        # erorr if there are really small values.
        alpha_new = solve(temp, t(R) %*% f, tol = 1e-22)
        ...
        # Setting tol = 1e-22 means when trying to solve for the inverse, it won't give an 
        # erorr if there are really small values.
        F_lambda = solve(temp, tol = 1e-22)%*%crossprod(R)

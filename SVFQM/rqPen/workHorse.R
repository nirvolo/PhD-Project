#####################################################################################
# **Found the source code at https://github.com/cran/rqPen/blob/master/R/workHorse.R
#####################################################################################
...
predModels_new <- function(object, newx){
...
check.errors_new <- function(object,newx,newy){
  lapply(object$models,check.errors.model_new,newx,newy)
}

check.errors.model_new <- function(object,newx,newy){
  preds <- predModels_new(object,newx)
  errors <- newy- preds
...
rq.enet_new <- function(x,y,tau=.5,lambda=NULL,nlambda=100,eps=ifelse(nrow(x)<ncol(x),.01,.0001), 
	...
	returnVal <- rq.lasso.huber_new(x,y,tau,lambda,penalty.factor,scalex,tau.penalty.factor,a=a,max.iter=max.iter,converge.eps=converge.eps,lambda.discard=lambda.discard,...)
	avals <- sapply(returnVal$models,modelA)
	tauvals <- sapply(returnVal$models,modelTau)
...
rq.pen.modelreturn_new <- function(coefs,x,y,tau,lambda,local.penalty.factor,penalty,a,weights=NULL){
	...
	x_names <- c(x_names)
	fits <- x%*% return_val$coefficients
	res <- y - fits
	if(is.null(dim(return_val$coefficients))==TRUE){
		...
		for(i in 1:length(return_val$rho)){
			return_val$PenRho[i] <- return_val$rho[i] + sum(penfunc(return_val$coefficients[,i],lambda[i]*local.penalty.factor,a))
		}
...
rq.lasso.huber.onetau_new <- function(x,y,tau,lambda,penalty.factor=rep(1,ncol(x)),scalex=TRUE,max.iter,converge.eps,a=1,lambda.discard,...){
	dims <- dim(x)
	p <- dims[2]
	if(scalex){
	...
	rq.pen.modelreturn_new(hqModel$beta,x,y,tau,lambda,penalty.factor,"LASSO",a)
}

rq.lasso.huber_new <- function(x,y,tau,lambda,penalty.factor=rep(1,ncol(x)),scalex=TRUE,tau.penalty.factor=rep(1,length(tau)),max.iter,converge.eps,a=1,lambda.discard,...){
	dims <- dim(x)
	n <- dims[1]
	p <- dims[2]
	nt <- length(tau)
	...
	for(i in 1:nt){
		...
		for(j in 1:na){			
			models[[pos]] <- rq.lasso.huber.onetau_new(x,y,tau=subtau,lambda=lambda,penalty.factor=penf,scalex=scalex,max.iter,converge.eps,a=a[j],lambda.discard,...)
			pos <- pos+1
